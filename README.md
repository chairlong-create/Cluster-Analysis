...
- 建类批次 `seed`
  - 推荐直接点击 `一键分类`
  - 系统会自动完成信号提取、聚类建议、确认入表和批量分类
  - 页面会显示当前进度和每一步状态
  - 仍保留手动提取、聚类、确认、分类的分步能力，便于排查和精细控制
- 直接分类批次 `classify_only`
  - 推荐直接点击 `一键分类`
  - 系统会跳过建类步骤，直接使用当前类别表分类

### 任务级处理其他

- `处理全部其他` 是任务级动作，不按批次分别处理
- 系统会汇总当前任务里所有落入 `其他` 的记录，统一重新提取、聚类、确认和重分
- 如果提取阶段部分失败，已成功提取的记录会继续往后推进
- 提取失败或没有有效信号的记录会继续保留在 `其他`，不会变成用户找不到入口的 `未分类`
- 如果中途在步骤切换处中断，页面会自动续跑当前轮次
- 如果续跑失败，会写入明确的 failed 记录，避免页面长期卡在处理中
...
- 点击后前端主动请求 CSV 并触发文件下载
- 按钮会显示“正在导出...”状态
- 请求失败时会在按钮下方显示错误提示
- CSV 带 UTF-8 BOM，中文内容和中文文件名对 Excel 更友好
...
src/components
  工作台组件、表格、交互控件、一键分类与导出按钮

src/lib
  SQLite 数据层、LLM provider、流程 service、一键分类编排、导出工具、配置读取
...
- 各步骤结构化 JSON key 仍是固定契约，Prompt 可以改分析口径，不要改 key 名
